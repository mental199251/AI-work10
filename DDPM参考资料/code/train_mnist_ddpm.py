...
OUTPUT_DIR = PROJECT_DIR / "outputs" / "mnist"
...
def main():
    ...
    model = SimpleUNet(image_channels=1, base_channels=BASE_CHANNELS).to(device)
    ...
    print("Required task: train MNIST DDPM")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        latest_path = output_dir / "mnist_ddpm_latest.pt"
        save_checkpoint(latest_path, model, optimizer, epoch, global_step, config)
        save_checkpoint(output_dir / f"mnist_ddpm_epoch_{epoch}.pt", model, optimizer, epoch, global_step, config)

        if SAMPLE_EVERY_EPOCH:
            samples, _ = diffusion.sample(model, shape=(16, 1, 32, 32), save_every=50, return_frames=False)
            save_tensor_grid(samples, output_dir / f"sample_epoch_{epoch}.png", nrow=4)

    elapsed = time.time() - start_time
    print(f"Done. Latest checkpoint: {output_dir / 'mnist_ddpm_latest.pt'}")
